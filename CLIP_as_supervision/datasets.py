...
class DataAugmentationForBEiT(object):
    def __init__(self, args):
        ...
        if args.discrete_vae_type == "dall-e":
            ...
            self.visual_token_transform = transforms.Compose([
                transforms.ToTensor(), ])
        elif args.discrete_vae_type in ["PatchVQGAN", 'ViTVQGAN', 'ViTCNNVQGAN', 'ViTVQGANITER', 'CONFORMERVQGAN', 'CONFORMERVQGAN_p14']:
            self.visual_token_transform = transforms.Compose([
                transforms.ToTensor(),])                                             
        else:
...
# for tokenzier traning
def build_vqgan_dataset(is_train, args):
    if is_train:
    ...
    if args.load_tar:
        return ImageDataset(os.path.join(args.data_path, 'train.tar' if is_train else 'val.tar'),
                            transform=transform)
    if is_train:
        if args.data_set == 'image_folder': # for 21k
            return ImageFolder(args.data_path, transform=transform)
        else: # for 1k, or other small datasets
            return ImageFolder(os.path.join(args.data_path, 'train'), transform=transform)
    else:
        if args.eval_data_path == '':
            return ImageFolder(os.path.join(args.data_path, 'val'), transform=transform)
        else:
