...
import util_itpn.lr_decay as lrd
import util_itpn.misc as misc
from util_itpn.pos_embed import interpolate_pos_embed
from util_itpn.misc import NativeScalerWithGradNormCount as NativeScaler
...
def get_args():
    parser = argparse.ArgumentParser('iTPN fine-tuning and evaluation script for image classification', add_help=False)
    ...
    parser.add_argument('--data_set', default='IMNET', choices=['CIFAR', 'IMNET', 'IMNET_l', 'image_folder'],
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        if ('absolute_pos_embed' in checkpoint_model) and (model.absolute_pos_embed is not None):
    ...
    if args.distributed:
        ...
        # No weight decay here, as they have been added in `lrd.param_groups_lrd`
