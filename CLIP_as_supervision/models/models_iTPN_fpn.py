...
class HiRep_Fpn(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=512, mlp_depth1=3, mlp_depth2=3, depth=24,
                 fpn_dim=512, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4., fpn_depth=2, init_values=0.1,
    ...
    def forward_features(self, x, ids_keep=None, mask=None):
        ...
        if rpe_index is None and self.num_outs > 3:
            ...
            #        B, Hp * out.shape[-3], Wp * out.shape[-2], -1
            #    ).permute(0, 3, 1, 2)
            #    for out in outs]
            if self.num_outs >= 4:
    ...
    """
    It is used for pretraining hirep_fpn as the backbone of beit.
    If it is used as for other tasks, keeping fpn, if used for classification, discards fpn
    using hirep.
    mask: useless for now, used in `Attention` module to add `inf` in softmax when calculating the attention matrix.
    
    Transfer beit-forward_features into here:
    1. add `masking` the whole image feature (after patch embed)
    2. mask_token is defined in `model_hirep_mae_fpn`, and will be used here.
    3. No cls_tokens
    """

    def forward_features_for_beit(self, x, bool_masked_pos, mask=None):
        B, C, H, W = x.shape
