...
from .models_hirep_fpn import HiRep_Fpn, PatchEmbed, PatchMerge, PatchSplit, BlockWithRPE
from util_hirep.pos_embed import get_2d_sincos_pos_embed


class HiRepMaskedAutoencoderFpn(MaskedAutoencoder, HiRep_Fpn):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, fpn_dim=512, fpn_depth=2,
                 embed_dim=512, mlp_depth1=2, mlp_depth2=2, depth=20, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4.,
    ...
    def forward_decoder(self, inps, ids_restore):
        ...
        for i, blk in enumerate(self.decoder_blocks):
            # if i in self.decoder_sidein and len(feats) > 0:
            #     x = x + feats.pop(0)
            x = blk(x)
...
def mae_hirep_base_fpn_dec512d8b(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
...
def mae_hirep_base_fpn1024_fpnd2_dec512d8b(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
        embed_dim=512, mlp_depth=3, depth=24, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4.,
        num_outs=3, decoder_sidein=[2, 4], fpn_dim=1024, fpn_depth=2,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def mae_hirep_base_fpn512_fpnd2_dec512d8b(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
        embed_dim=512, mlp_depth=3, depth=24, num_heads=8, bridge_mlp_ratio=3., mlp_ratio=4.,
        num_outs=3, decoder_sidein=[2, 4], fpn_dim=512, fpn_depth=2,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model


def mae_hirep_fpn_768_2240(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
        embed_dim=768, mlp_depth1=2, mlp_depth2=2, depth=40, num_heads=12, bridge_mlp_ratio=3., mlp_ratio=4.,
        num_outs=3, decoder_sidein=[2, 4],
...
def mae_hirep_fpn_1024_2322(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
        embed_dim=1024, mlp_depth1=2, mlp_depth2=3, depth=22, num_heads=16, bridge_mlp_ratio=3., mlp_ratio=4.,
        num_outs=3, decoder_sidein=[2, 4],
...
def mae_hirep_fpn_896_2330(**kwargs):
    model = HiRepMaskedAutoencoderFpn(
        embed_dim=896, mlp_depth1=2, mlp_depth2=3, depth=30, num_heads=14, bridge_mlp_ratio=3., mlp_ratio=4.,
        num_outs=3, decoder_sidein=[2, 4],
        decoder_embed_dim=896, decoder_depth=8, decoder_num_heads=16,
        rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
