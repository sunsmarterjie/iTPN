...
class iTPNClipDistill(MaskedAutoencoder, iTPN):
    ...
    def forward(self, x, bool_masked_pos, return_all_tokens=False, return_patch_tokens=False):
        # forward_features_for_beit is in `models_hirep_fpn`
        outs, aux_out = self.forward_features_for_beit(x, bool_masked_pos=bool_masked_pos)
        ...
        return [self.lm_head(x), self.lm_cls_head(aux_x)]
